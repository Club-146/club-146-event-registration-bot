...
async def _build_event_choices(
    app: App, active_events: list[dict], archived_events: list[dict]
) -> dict[str, str]:
    """Build the choices dict from active/archived events for the management menu."""
...
async def _handle_archive_selection(
    chat_id: int,
    state: FSMContext,
    app: App,
    archived_events: list[dict],
) -> None:
    """Handle the archive submenu: show archived events and allow unarchiving."""
    if not archived_events:
        await send_safe(chat_id, "Архив пуст.")
        return
...
async def _handle_event_action(
    ...
    reg_count = await app.get_registration_count_for_event(event_id)
    ...
    if action == "toggle":
        ...
        await _handle_archive_event(
            chat_id, state, app, event, event_id, reg_count, user_id, username
        )
    elif action == "edit":
