...
class TestUpdateEventStatusesArchiveLogic:
    @pytest.mark.asyncio
    async def test_archive_cutoff_query(self, app):
        """Verify archive query uses 3-month cutoff and correct statuses."""
        calls = []

        async def track_update_many(query, update):
            calls.append((query, update))
            return MagicMock(modified_count=0)

        app.events_col.update_many = track_update_many
        await app._update_event_statuses()
        assert len(calls) == 2
        # First call = archive
        archive_query = calls[0][0]
        assert "passed" in archive_query["status"]["$in"]
        assert "upcoming" in archive_query["status"]["$in"]
        archive_update = calls[0][1]["$set"]
        assert archive_update["status"] == "archived"
        assert archive_update["enabled"] is False

    @pytest.mark.asyncio
    async def test_archive_does_not_include_already_archived(self, app):
        """Archived events should not be in the archive query filter."""
        calls = []
        ...
        app.events_col.update_many = track_update_many
        await app._update_event_statuses()
        archive_statuses = calls[0][0]["status"]["$in"]
        assert "archived" not in archive_statuses


class TestExportDebounceDetails:
    @pytest.mark.asyncio
    async def test_force_returns_result(self, app):
        app.sheet_exporter = MagicMock()
        app.sheet_exporter.export_registered_users = AsyncMock(return_value="done")
        result = await app.export_registered_users_to_google_sheets(force=True)
        assert result == "done"
        assert app._export_debounce_task is None

    @pytest.mark.asyncio
    async def test_debounce_cancels_previous(self, app):
        import asyncio
        ...
        # First call
        await app.export_registered_users_to_google_sheets(event_id="a")
        first_task = app._export_debounce_task
        # Second call cancels first
        await app.export_registered_users_to_google_sheets(event_id="b")
        assert first_task.cancelled()
        await asyncio.sleep(0.3)
        # Only the second export should have fired
        app.sheet_exporter.export_registered_users.assert_called_once_with(event_id="b")

    @pytest.mark.asyncio
    async def test_debounce_returns_none(self, app):
        app.sheet_exporter = MagicMock()
        app.sheet_exporter.export_registered_users = AsyncMock()
        app._export_debounce_seconds = 0.1
